...
from calendar_app.models import Event, EventMarker
#from .forms import UpdateFormProfile, UpdatePasswordForm
...
class GroupDetail(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        event_list = Event.event_manager.get_events_by_date_and_groups(curr_date, [group_pk])
        every_week_events = Event.event_manager.get_every_week_events([group_pk])
        ...
        studens_list = [student.pk for student in group.students.all()]
        event_list_pk = [pk['event_pk'] for pk in event_list]
        #print(studens_list)
        #print(event_list_pk)

        eventmarker = EventMarker.event_marker_manager.get_events_by_students_and_events(students=studens_list, events=event_list_pk)
        
        marker_list = [[marks.student.pk, marks.event.pk] for marks in eventmarker]
        marker_list = ''.join(map(str, marker_list)).replace(' ', '').replace('[', ' ').replace(']', ' ')
        print(marker_list)
        context = {
            'data': mentor_profile,
            'group': group,
            'event_list': event_list,
            'event_marks': marker_list,
        }
        if is_ajax(request):
            return JsonResponse({'event_marks': marker_list}, status=200)
    ...
    def post(self, request, *args, **kwargs):
        if request.POST.get('student-delete'):
        ...
        elif request.POST.get('counter') and is_ajax(request):
            counter = request.POST.get('counter')
            type_of_add = request.POST.get('add-type')
            student_pk = request.POST.get('student-add')
            event_pk = request.POST.get('event-add')
            student = StudentProfile.objects.get(pk=student_pk)
            event = Event.objects.get(pk=event_pk)
            group = Group.objects.get(pk=kwargs['pk'])
            eventmarker = EventMarker.objects.filter(student=student, event=event)
            ...
            if len(eventmarker) == 1:
                if eventmarker[0].dates.count(datetime.date.today()) == 0:
                    eventmarker[0].dates.append(datetime.date.today())
                    eventmarker[0].mark = True
                    eventmarker[0].save()
            elif len(eventmarker) < 1:
                EventMarker.objects.create(student=student, event=event, dates=[datetime.date.today()], mark=True)
            else:
                return HttpResponseServerError()
            ...
            student.save()
            return JsonResponse({'status': 'Request is done', 'event': event_pk}, status=200)
        else:
            print('is_not_ajax')
...
class ProfileForStudent(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        invite_group = request.session.get('invite_group_id', '/')
        if invite_group != "/":
            student = request.user.studentprofile
            group = Group.objects.get(pk=invite_group)
            if group.students.filter(pk=student.pk):
                print("Вы уже состоите в данной группе")
            else:
                group.students.add(request.user.studentprofile)
                print("Студент успешно добавлен в группу")
                request.session['invite_group_id'] = "/"
                request.session['invite_group_name'] = "name"
            print(invite_group)
...
class ProfileUpdater(LoginRequiredMixin, View):
    raise_exception = True
    template_name = "user_profile/settings.html"
    def get(self, request, *args, **kwargs):
        mentor_profile = request.user.mentorprofile
...
class Chat(LoginRequiredMixin, View):
    raise_exception = True
    template_name = "user_profile/chat.html"
    ...
    def post(self, request, *args, **kwargs):
        return redirect('user_profile:chat')


class Offer(LoginRequiredMixin, View):
    raise_exception = True
    template_name = "user_profile/offer.html"
...
class MentorDashboard(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect('user_profile:mentor_groups')
...
def invite_group(request, name, pk):
    if request.user.is_authenticated:
        ...
        group.students.add(request.user.studentprofile)
        return HttpResponse("хы")
    else:
        request.session['invite_group_id'] = pk
        request.session['invite_group_name'] = name
