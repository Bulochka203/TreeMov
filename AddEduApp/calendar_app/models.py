...
class EventMarkerQuerySet(models.QuerySet):
    def get_events_by_students_and_events(self, students, events):
        return self.filter(student__in=students, event__in=events)
    def get_events_by_student(self, student):
        return self.filter(student=student)


class EventMarkerManager(models.Manager):
    def get_queryset(self):
        return EventMarkerQuerySet(self.model)

    def get_events_by_students_and_events(self, students, events):
        return self.get_queryset().get_events_by_students_and_events(students, events)


class EventMarker(models.Model):
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE)
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name='eventmarker')
    dates = ArrayField(models.DateField())
    mark = models.BooleanField()

    objects = models.Manager()
    event_marker_manager = EventMarkerManager()
    ...
    def template_output(self):
        return {
            ...
            'event': self.event,
            'date': datetime.strftime(self.dates, "%Y-%m-%d"),
            'mark':self.mark,
        }